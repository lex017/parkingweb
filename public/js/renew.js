document.addEventListener("DOMContentLoaded", function () {
    ...
    querySnapshot.forEach((doc) => {
        ...
        // Show only pending status and Renew package
        if (userData.package !== "Renew") {
        ...
        row.innerHTML = `
            ...
            <td>${userData.package || "N/A"}</td>
            <td>${userData.tag || "N/A"}</td>
            <td>${formattedTimestamp}</td>
            <td>
                <button class="verify-btn" data-id="${billId}">
                    Verify
                </button>
                <button class="reject-btn" data-id="${billId}">
                    Reject
                </button>
    ...
    // ✅ Attach button listeners as before
...
function verifyPayment(billId) {
    ...
    bookingRef.get().then((billDoc) => {
        ...
        bookingRef.update({ status: "renew" })
            .then(() => paymentRef.update({ status: "renew" }))
